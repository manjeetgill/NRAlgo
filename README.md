...
2. Distinguish **Server capability** from temporary **Live trading** permission. Both default to off. If the server is locked, broker login cannot unlock it. The operator must satisfy configuration/static-IP requirements first. Enabling a configured account requires a fresh authenticator/recovery code and the exact confirmation phrase; it expires after five minutes. Disabling remains an immediate no-2FA kill switch so loss of an authenticator cannot prevent risk reduction.
