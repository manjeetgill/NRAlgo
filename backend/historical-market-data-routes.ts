...
export function registerHistoricalMarketDataRoutes(
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      /** Closing the browser response cancels queued work and the provider transport. */
      ...
      res.once("close", cancelClosedResponse);
      ...
      try {
        await coordinator.runQueuedForUser(
          session.user_id,
          cancellation.signal,
          async () => {
            ...
            const contract = (() => {
              try {
                return provider.instruments.resolve(input.market, input);
              } catch {
                return fail(
                  422,
        ...
        res.removeListener("close", cancelClosedResponse);
      }
    },
  );
}
